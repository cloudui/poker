<script lang="ts">
  ...
  async function nextRound() {
    try {
      const response = await fetch("http://localhost:8000/next_round", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
<div class="page max-w-4xl mx-auto pt-8">
  ...
  {#if !gameStarted}
    <button type="button" class="btn" on:click={handleStartGame}>
      Start Game
    </button>
  {/if}
          ...
          >
            ...
            <div class="flex items-center gap-4">
              <div class="avatar">
                ...
                >
                  <img
                    src="https://api.dicebear.com/6.x/bottts/svg?seed={turn
                      .player.name}"
                    alt={turn.player.name}
                  />
                </div>
              </div>
              <div>
